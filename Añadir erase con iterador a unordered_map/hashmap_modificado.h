...
#ifndef hashmap_eda_modificado_h
#define hashmap_eda_modificado_h
...
template <class Clave, class Valor,
	...
	protected:
		using umap_t = unordered_map_modif<Clave, Valor, Hash, Pred>;
	...
	public:
		...
		// constructor
		unordered_map_modif(int n = TAM_INICIAL, Hash h = Hash(), Pred p = Pred()) :
			array(n, nullptr), nelems(0), hash(h), pred(p) {}

		// constructor por copia
		unordered_map_modif(umap_t const& other) {
			copia(other);
		}
	...
	protected:
		...
		protected:
			friend class unordered_map_modif;
			pointer_tabla tabla;   // la tabla que se está recorriendo
			Link act,ant;        // nodo actual
	...
	public:
		...
		iterator erase(iterator it) {
			if (it.act == nullptr) throw std::out_of_range("Iterador fuera de la lista");
			iterator sol= it;
			++sol;
			if (sol.ind == it.ind)
				sol.ant = it.ant;
